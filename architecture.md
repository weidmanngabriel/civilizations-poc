...
Das Lab rendert mit der über Vite gebündelten Projektabhängigkeit Three.js einen einfachen blockigen Referenzcharakter über eine orthografische isometrische Kamera; zur Laufzeit ist dafür kein CDN nötig. Körperteile hängen an festen lokalen Pivots; erlaubte Rotationsachsen und Winkelgrenzen liegen in der Character-Definition. Animationen sind JSON-Daten mit normiertem Fortschritt von 0 bis 1, absoluten lokalen Gelenkwinkeln und einer wählbaren Interpolationskurve (`linear`, `easeIn`, `easeOut`, `easeInOut`). Einzelne Keyframes dürfen die Interpolation des jeweils folgenden Segments überschreiben; Root-Motion (`root.x`, `root.z`, `root.yaw`) ist für räumliche Vorschauabläufe im Character Lab ebenfalls erlaubt. Zwischen Keyframes wird linear interpoliert.
...
Character Lab CI additionally produces deterministic PNG review frames from the built site using headless Chrome. These are uploaded as workflow artifacts so animation changes can be visually inspected without coupling the review tooling to the game runtime.
