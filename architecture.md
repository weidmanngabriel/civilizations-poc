...
- `src/simulation/model.ts` defines a generic person, assignment, transport trip, recipe, building and world state. Roles are data, not subclasses. Forest buildings may now use dynamic IDs (`forest-N`) and carry finite forest state.
- `scenario.ts` owns the fixed **21 × 13** offset-layout hex map, axial building coordinates, eight-person start and economy configuration. Buildings still occupy one tile, while the road network deliberately separates production stages by longer routes and includes branches/loops. The map also contains several small groups of walkable passive forest tiles.
...
- `simulation.ts` owns deterministic in-place ticks, assignment changes, population changes, reservations, recipes, finite forest depletion, forest relocation and status derivation. No timers or renderer imports occur in the core.
- Each tick moves every person at most once, handles arrivals and transfers, advances production once, handles exhausted-forest relocation, then plans new transport trips. Stable person/building order resolves non-random ties.
...
- Every active forest starts with `CONFIG.forestYield` (= 10) units of remaining harvest. In-progress forest workers reserve remaining yield implicitly, so parallel workers cannot produce beyond the finite total.
- Once a forest reaches zero remaining yield, each assigned forest worker independently chooses among the nearest reachable passive forest tiles. Equal-distance choices use a small seeded PRNG stored in `World.rngState`, preserving deterministic replay while still producing pseudo-random splits. Up to two relocating workers may select the same destination. Each unique chosen tile becomes a new active forest building with a fresh yield of 10.
- An exhausted forest keeps its already produced wood available as a normal logistics source. As soon as its local output reaches zero and its workers have relocated, the visible forest retires and its tile becomes a road, even if the last picked-up wood is still in transit. The retired building record remains internally so transport references stay valid. If a carried unit is explicitly cancelled and returned to that source, the depleted forest is restored as a visible rest-wood source until that returned output is collected again.
- `game/MainScene.ts` renders the 21 × 13 world into a 1000 × 570 Phaser canvas using a denser hex projection, plus numbered person markers and live input/output capacity slots. Terrain and building layers are redrawn from world state so passive forests, newly activated forests and retired road tiles update immediately.
