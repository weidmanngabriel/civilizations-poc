...
- Automatic simulation ticks and presentation refreshes are deliberately decoupled. Normal 1–10 FPS autoplay still refreshes after each tick when no control pointer is currently pressed. **Max FPS** continues to advance one simulation round per `requestAnimationFrame`, but the expensive DOM/Phaser presentation refresh is capped at one update per 100 ms. Automatic refreshes are suppressed while a pointer is held on a UI control so re-rendering cannot replace a button between pointer-down and click. User commands and pausing still force an immediate refresh.
