...
Every browser save receives a 16:9 settlement preview generated by `MainScene.captureSettlementThumbnail()`. The capture temporarily frames all non-retired, non-field buildings (including HQ and construction sites), with extra visual padding, renders a small WebP thumbnail, and then restores the player's previous camera position and zoom. Roads, residents, resources and fields do not expand the framing bounds.
