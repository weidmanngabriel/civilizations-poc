...
Wildlife is authoritative simulation state in `World.animals` and `World.animalGroups`. Species behavior is isolated in `src/simulation/wildlife.ts`; the first species is the hare, but movement cadence, flock/home attraction, flight duration and speed are species data rather than hunter-specific branches. Hare groups keep a home point plus a soft group migration target while every animal moves independently. About every 30 simulated seconds the group target moves 10–15 micro-cells from the current group center. Consecutive targets retain directional inertia instead of choosing an unrelated direction each time, producing longer wandering arcs. The original home point is only a soft outer boundary: it exerts no pull while the group center remains within roughly 30 micro-cells and starts attracting the group back only beyond that range. Individual path choice is influenced only lightly by that target. Hares keep a loose personal spacing: inside roughly five micro-cells there is no pull toward the flock center, and very close neighbors create a separation bias; only animals that drift farther away are pulled back toward the group. Current animal cells and already planned endpoints are excluded from new path endpoints, and a blocked next cell triggers immediate replanning, so two hares never intentionally finish on the same micro-cell. This keeps the group drifting without collapsing into a tight formation. Normal movement starts every 4–8 simulated seconds and uses short zig-zag paths. A shot makes every surviving member of the attacked group flee independently for five simulated seconds; after that, the normal flock/home/group-target weighting resumes so scattered groups gradually gather again.

Hunters are outdoor workers with a personal flag. Their radius is 10 coarse world tiles / 50 micro-cells, four times the normal extractor/fisher radius. Bow range remains a separate fixed 2-world-tile / 10-micro-cell value, so enlarging the hunting area does not enlarge the weapon. Hunters autonomously select wildlife only inside the hunting area and use the normal person path system to move into bow range. Once a target is in range, the hunter stops and aims for two simulated seconds. That aiming cycle locks the target: if the animal leaves bow range during those two seconds, the shot still fires when aiming completes. After the shot the hunter reevaluates distance before beginning the next aiming cycle.

Ranged attacks are separated from hunting in `src/simulation/rangedCombat.ts`. A ranged attack creates an authoritative projectile containing source/target entity references, launch/impact ticks, projectile kind, impact lifetime and the already resolved hit roll. Impact effects resolve exactly once; arrows then remain authoritative world objects at their impact position for ten simulated seconds before expiring. Presentation interpolates flying projectiles and renders impacted arrows in world scale without owning authoritative positions. Hare rendering uses a reusable texture and interpolates each current micro-cell toward the next path cell using the simulation movement fraction, with a presentation-only hop arc; sprite size stays in world units so zoom makes the animal visibly larger. This is intentionally reusable for later ranged soldiers: hunter targeting and XP stay in `hunting.ts`, while projectile timing and attack transport remain generic. Hunter bow accuracy scales linearly from 20% at 0 experience to 95% at 100 experience; firing at an already fleeing animal multiplies that chance by 0.5. Hunter experience is awarded only when a projectile actually kills wildlife. A killed hare creates one physical `meat` unit at or near its final position. That stack is reserved for the successful hunter, who walks to it, performs the normal one-second ground pickup, carries it as outdoor cargo back to the personal flag and deposits it as an ordinary loose-good stack before hunting again. Meat then participates in normal storage/transport and restores the same 60 hunger points as fish.
