...
Wildlife is authoritative simulation state in `World.animals` and `World.animalGroups`. Species behavior is isolated in `src/simulation/wildlife.ts`; the first species is the hare, but movement cadence, flock/home attraction, flight duration and speed are species data rather than hunter-specific branches. Hare groups keep a home point plus a soft group migration target while every animal moves independently. About every 30 simulated seconds the group target moves 10–15 micro-cells from the current group center. Individual path choice is influenced only lightly by that target; attraction to the current flock center remains strongest, followed by home attraction and individual randomness, so the group drifts without moving as a formation. Normal movement starts every 4–8 simulated seconds and uses short zig-zag paths. A shot makes every surviving member of the attacked group flee independently for five simulated seconds; after that, the normal flock/home/group-target weighting resumes so scattered groups gradually gather again.
