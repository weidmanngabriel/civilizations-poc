...
export function mountControls(w: World, renderMap: () => void): void {
  ...
  app.innerHTML = `<main><header><div><p class="eyebrow">DAS ACHTE WELTWUNDER / POC 01</p><h1>Ein Dorf kommt in Gang.</h1><p class="intro">Verteile die Menschen. Verbinde die Wirtschaft. Eine Runde nach der anderen.</p></div><span class="badge">Produktionslogistik</span></header><section class="toolbar"><div id="metrics"></div><div class="round-controls"><button id="next" class="primary">Nächste Runde <span aria-hidden="true">→</span></button><button id="autoplay" aria-pressed="false">Autolauf starten</button><label class="speed-control">Geschwindigkeit <input id="fps" type="range" min="1" max="10" step="1" value="1" aria-label="Autolauf in Runden pro Sekunde"><output id="fps-value">1 FPS</output></label></div></section><section class="map-panel"><div class="map-heading"><span>Das erste Dorf</span><span>Wald → Sägewerk → Schreinerei → Lager</span></div><div id="game" role="img" aria-label="Große Hex-Karte mit Hauptquartier, Waldflächen, Sägewerk, Schreinerei und Lager. Personen bewegen sich auf Wegen, Wald- und Gebäudefeldern."></div><div class="legend"><span><i class="worker"></i> Arbeiter</span><span><i class="carrier"></i> Träger</span><span><i class="free"></i> Frei</span><span>Fracht: H Holz · B Brett · W Werkzeug</span><span>Waldflächen sind begehbar und liefern je Standort 10 Holz.</span><span>Wiese, Berg und Fluss sind gesperrt.</span></div></section><section class="population"><div><strong>Bevölkerung</strong><small>Debug · Entfernen nur bei freien Personen am HQ</small></div><div class="stepper"><button id="population-minus" aria-label="Bevölkerung verringern">−</button><output id="population-count"></output><button id="population-plus" aria-label="Bevölkerung erhöhen">+</button></div></section><section class="section-title"><h2>Arbeitsstätten</h2><p>Besetzung: zugewiesen / Maximum · aktiv nach Ankunft</p></section><div id="buildings" class="cards"></div><p class="hint">Zum Start: 2 Arbeiter im Wald, je 1 im Sägewerk und in der Schreinerei, dazu mindestens 1 Träger im Lager. Erschöpfte Waldarbeiter wechseln automatisch zu nahen Waldflächen.</p><details><summary>Personen und Transportaufträge</summary><div id="people"></div></details><footer>PoC 1 · Jede Wegkante kostet eine Runde. Produktion benötigt fünf Arbeitsrunden.</footer></main>`;
    ...
    document.querySelector("#buildings")!.innerHTML = w.buildings
      ...
      .map((b, index) => {
        ...
        const recipe = b.forestRemaining !== undefined
          ? `1 Holz je Arbeiter / 5 Runden · Vorrat ${b.forestRemaining}/${CONFIG.forestYield}`
          : b.recipe?.input
