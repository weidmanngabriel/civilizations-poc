    ...
    const candidates = w.naturalResources
      ...
      .map((resource) => {
        const path = w.wayposts === undefined
          ...
          : findRequiredNavigationPath(w, person, resource.position, CONFIG.roadSpeedMultiplier);
        return path ? {
          resource,
          path,
    ...
    const candidates = w.buildings
      ...
      .map((site) => {
        const path = w.wayposts === undefined
          ...
          : findRequiredNavigationPath(w, person, site.position, CONFIG.roadSpeedMultiplier);
        return path
          ? {
              site,
              path,
      ...
      for (const source of w.buildings) {
        ...
        const path = performanceProfiler.withPathReason(pathReason, () =>
          w.wayposts === undefined
            ...
            : findRequiredNavigationPath(w, p, source.position, CONFIG.roadSpeedMultiplier),
        );
  ...
  if (
    ...
    !performanceProfiler.withPathReason("merchant", () =>
      w.wayposts === undefined
        ...
        : findRequiredNavigationPath(w, p, target.position, CONFIG.roadSpeedMultiplier),
    )
  )
    return false;
  p.trip = {
