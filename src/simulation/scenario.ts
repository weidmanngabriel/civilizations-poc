...
function createScenario({ population, suppliedStart }: ScenarioOptions): World {
  ...
  const people: Person[] = Array.from({ length: population }, (_, i) => ({
    ...
    sex: ((Math.imul(i + 1, 1103515245) + 12345) >>> 30) & 1 ? "male" : "female",
    ageStage: "adult",
    parentIds: [],
    childIds: [],
    hunger: 100,
  ...
  if (suppliedStart) {
    ...
    spawnAnimalGroup(world, "cow", scaledAt(8, 15), 3);
    spawnAnimalGroup(world, "cow", scaledAt(31, 3), 4);
    spawnAnimalGroup(world, "sheep", scaledAt(14, 8), 5);
    spawnAnimalGroup(world, "sheep", scaledAt(35, 15), 6);
  }
