...
Das Hauptspiel definiert zentral, welche visuellen Gebäudevarianten existieren. Dazu gehören insbesondere Wohnhaus 1–5 sowie die bereits getrennten Spielstufen Töpferei 1/2 und Steinmetzhütte 1/2. Der Editor zeigt genau diese Varianten in einem alphabetisch nach deutschem Anzeigenamen sortierten Dropdown und kann selbst keine neuen Spielstufen anlegen oder löschen. Jeder Listeneintrag trägt einen Status: **✓** bedeutet, dass für genau diese Variante bereits Sprite und Plan hinterlegt sind; **○ … · Platzhalter** bedeutet, dass sie noch nicht konfiguriert ist. Das kann sowohl einen kompletten Placeholder-Slot als auch nur eine noch fehlende Stufe innerhalb eines teilweise konfigurierten Gebäudes betreffen. Bei Auswahl lädt der Editor ausschließlich die konkrete Variante mit Sprite, Anchor, Weltbreite, Grundriss, blockierten Zellen und Eingang. Palisaden bleiben als Infrastruktur außerhalb dieses generischen Gebäudeeditors.

Im veröffentlichten Editor werden `building.json` und das **unveränderte** Sprite heruntergeladen. Der Export verkleinert oder recomprimiert die gewählte Bilddatei nicht. Dieses Dateipaar kann gemeinsam wieder importiert und vollständig weiterbearbeitet werden, sofern es dem aktuellen Schema entspricht. Bei lokaler Entwicklung wird die bearbeitete Variante in die bestehende `building.json` des zugehörigen Asset-Slots unter `src/assets/buildings/<kind>/` integriert und nur ihr Sprite geschrieben; andere bereits konfigurierte Varianten des Slots bleiben erhalten. Bei neu angelegten, nicht aus dem Projekt geladenen Definitionen dient weiterhin die Definition-ID als neuer Slotname.
