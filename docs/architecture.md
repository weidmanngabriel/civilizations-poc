...
The fine-grid base map is still authored through the existing `MainScene.drawMap()` rules, but `IncrementalMainScene` does not leave that large Graphics command list in the normal render path. Whenever the map signature changes it redraws the authoritative map Graphics once, copies the result into a world-sized Phaser `RenderTexture`, then hides the source Graphics. Normal frames therefore render one cached map texture instead of re-rendering tens of thousands of hex polygons. Dynamic overlays, people, building sprites and interaction highlights remain separate Game Objects above that cache. The profiler records the cache refresh separately as `renderMapCache`.
