...
`src/buildings/buildingVisualVariants.ts` is the game-owned catalog of valid visual variants. It defines which kind/level combinations exist independently from editor-authored assets; the editor must not create additional gameplay levels. `src/buildings/buildingVisualDefinition.ts` defines version 4 of the shared editor/runtime spatial schema. A building definition owns a stable `id` plus the subset of catalog levels that already have authored visual data. Each level independently contains:
...
When the local editor saves a selected variant, the selected asset-slot key remains the write target even if the definition's internal `id` differs from the slot key. The edited level is merged with the slot's other already-authored levels; the editor cannot add a kind/level pair that is absent from the game catalog. Activating another editor-authored runtime building now consists only of replacing `building.json` and the sprite inside the existing same-key asset slot. No additional registry edit is required for current kinds because every current kind is already wired through `register(...)`. When a new `BuildingKind` is introduced in code, its same-key asset slot and registry registration are added in that same implementation run.
...
Building progression is monotonic. Unlock evaluation may combine production-chain prerequisites with resident qualification or profession-experience requirements, but once an unlock is written to world state it is never revoked when those prerequisites later disappear. Multi-level building types persist unlock state per level and evaluate every level independently. Residential progression follows this general rule: cumulative direct-build material requirements provide its production prerequisites, while `World.unlockedHouseLevels` persists the resulting per-level unlocks.
...
Autonomous births are household-level checks controlled by `low | medium | high` settlement policy. The checks are deterministic, use a five-minute post-birth cooldown, and create one/two/three children using the 90/9/1 distribution. Both parents route to the shared home and pause ordinary work while the short birth celebration runs. A birth journey keeps its existing navigation path while it is valid; family processing only plans the home route when no route is currently present, such as after a completed need interruption. Presentation reads short-lived `familyEffects` from world state for hearts/bird cues.
