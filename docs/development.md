...
Opening or updating that pull request automatically runs full validation through GitHub Actions. The pull-request run must pass `npm test`, `npm run build` and the production-browser startup smoke test `npm run smoke:startup`. If fixes are required after the pull request is open, push them to the same temporary branch; the `synchronize` event reruns validation and supersedes the older run. No commits may be added after the final successful validation, and the validated commit SHA must exactly match the PR head that is merged.

At the end of a run, squash-merge the change into `main` so exactly one meaningful commit remains for that adjustment. A push to `main` runs tests, build and the browser startup smoke again and deploys GitHub Pages only when all three succeed. Manual workflow runs do not deploy. After the squash merge, verify build and deployment status; fixes use a new temporary branch and another single squash commit.
